...
import { Search, Plus, Edit, Trash2, Tag, Archive, Eye, AlertTriangle, FileDown, FileUp, Flame, ArrowRight, History, Package, Box, RefreshCw } from 'lucide-react';
        ...
        <div className="p-8 h-full flex flex-col bg-[#f8fafc]">
            ...
            <div className="flex gap-4 mb-6">
                <button 
                    ...
                    className={`px-6 py-3 rounded-xl text-sm font-black transition-all border-2 ${activeTab === 'ALL' ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-400 border-transparent hover:bg-slate-50'}`}
                >
                    Todos los Productos
                </button>
                <button 
                    onClick={() => setActiveTab('REPLENISH')} 
                    className={`px-6 py-3 rounded-xl text-sm font-black transition-all border-2 flex items-center gap-2 ${activeTab === 'REPLENISH' ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-white text-slate-400 border-transparent hover:bg-slate-50'}`}
                >
